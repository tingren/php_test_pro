...
class Umeng {
	...
	function sendAndroidBroadcast($deviceToken,$text=null,$ticker=null,$title=null,$msg=null) {
		try {
	...
	function sendAndroidUnicast($deviceToken,$text=null,$ticker=null,$title=null,$extra=null) {
		try {
			if(empty($deviceToken) || empty($text) || empty($ticker) || empty($title)){
				return false;
			}
			...
			// Set extra fields
			//$unicast->setExtraField("test", "Alfk22R7Om-xjD7SKSSWT0EntfxslIF0go3PYcRFjk1c");
	...
	function sendIOSUnicast($deviceToken,$text=null,$ticker=null,$title=null,$extra=null) {
		try {
			...
			$badge = empty($extra)?0:$extra['unread'];
